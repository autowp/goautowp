...
func (s *Service) getURIByChatID(ctx context.Context, chatID int64) (*url.URL, error) {
	var userID int64
	success, err := s.db.Select(schema.TelegramChatTableUserIDCol).From(schema.TelegramChatTable).Where(schema.TelegramChatTableChatIDCol.Eq(chatID)).ScanValContext(ctx, &userID)
